...
try{
  ...
  await watermelon.click();await page.locator('.campaign-detail.is-open').waitFor({state:'visible'});await page.waitForTimeout(0);assert(await page.locator('.campaign-detail__run-button').count()===0,'locked location must not expose a run launcher');assert((await page.locator('.campaign-detail__note').textContent())?.toLowerCase().includes('previous'),'locked route should explain the previous-Landmark gate');await page.locator('.campaign-detail__close').click();
