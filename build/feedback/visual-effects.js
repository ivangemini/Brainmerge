...
function transientClass(element, className, duration = 620) {
    if (!(element instanceof HTMLElement) || !motionAllowed())
        return;
...
export function runUnitFlight(from, to, merge) {
    if (!motionAllowed() || !from || !to)
        return;
    const visual = from.querySelector('.unit-visual');
    if (!visual)
        return;
...
export function runCoinTrail(from, hud, amount) {
    if (!motionAllowed() || !from || amount <= 0)
        return;
...
function runSpawnEnergy(targetCell) {
    if (!motionAllowed())
        return;
...
export function runDiscoveryCelebration(cell, tier) {
    if (!motionAllowed() || !cell || tier < 2)
        return;
    ...
    const chip = document.querySelector(`.collection-chip[data-chain-tier="${tier}"]`);
    const collection = chip?.closest('.side-card--collection') ?? null;
...
export function elementFxCenter(element) {
    return centerOf(element);
}
function installPointerDragFx() {
    ...
    root.addEventListener('pointerdown', (event) => {
        if (!motionAllowed() || event.button !== 0)
            return;
        const cell = event.target instanceof Element ? event.target.closest('[data-cell].is-occupied') : null;
        if (!cell)
            return;
        active = cell;
...
function installMoveRejectFx() {
    const root = document.querySelector('#app');
    if (!root)
        return;
    root.addEventListener('pointerdown', (event) => {
        if (!motionAllowed() || event.button !== 0 || !(event.target instanceof Element))
            return;
        const target = event.target.closest('[data-cell]');
        const source = root.querySelector('[data-cell].is-selected');
        ...
        if (!targetOccupied) {
            ...
            window.setTimeout(() => {
                const landed = targetIndex === undefined ? null : root.querySelector(`[data-cell="${targetIndex}"]`);
...
function installPressFx() {
    ...
    root.addEventListener('pointerdown', (event) => {
        if (!motionAllowed() || event.button !== 0 || !(event.target instanceof Element))
            return;
        const control = event.target.closest('[data-action],.locale-button,.audio-toggle');
...
function installSpawnEnergyFx() {
    ...
    const observer = new MutationObserver((records) => {
        ...
        for (const record of records) {
            if (record.type !== 'attributes' || record.attributeName !== 'class' || !(record.target instanceof HTMLElement))
                continue;
...
function installProgressionFx() {
    ...
    const snapshot = () => {
        ...
        const nextMissionSignal = `${root.querySelector('.mission-row strong')?.textContent?.trim() ?? ''}|${root.querySelector('.mission-track i')?.style.width ?? ''}`;
        ...
        if (initialized && motionAllowed()) {
            if (nextLevel && levelText && nextLevel !== levelText)
                transientClass(root.querySelector('.hud-pill--level'), 'fx-level-up', 860);
            if (nextMissionSignal && missionSignal && nextMissionSignal !== missionSignal)
                transientClass(root.querySelector('.side-card--mission'), 'fx-mission-progress', 720);
            if (nextMove && nextMoveText && nextMove !== nextMoveText)
                transientClass(root.querySelector('.next-action'), 'fx-next-move', 620);
            if (nextMove && !nextMoveText)
                transientClass(root.querySelector('.next-action'), 'fx-next-move', 620);
            if (nextMissionComplete && !missionComplete)
                transientClass(root.querySelector('.side-card--mission'), 'fx-mission-complete', 980);
        }
        levelText = nextLevel;
