...
function locationConfig(locationId) {
    return LOCATION_CONFIGS[locationId] ?? null;
}
function landmarkLevelForRestore(progress) {
    if (progress >= 1)
        return 3;
    if (progress >= 2 / 3)
        return 2;
...
function sanitizeCampaignUnit(candidate) {
...
function phaseOvergrowthIndexes(phase, locationId = SNEAKER_GARDEN_LOCATION_ID, landmarkLevel = 0) {
...
function sanitizeOvergrowth(candidate, phase, locationId, landmarkLevel = 0) {
...
function initialCampaignCells(overgrowth, bonusTierOnes = 0) {
...
function capLocationOrderTier(maxDiscoveredTier, locationId = SNEAKER_GARDEN_LOCATION_ID) {
...
function defaultOrderTiers(phase, maxDiscoveredTier, locationId = SNEAKER_GARDEN_LOCATION_ID) {
    ...
    if (phase === 'restore')
        return [minTier, minTier, Math.min(maxTier, minTier + 1), Math.min(maxTier, minTier + 1), maxTier, maxTier];
    if (phase === 'mastery')
        return [Math.min(maxTier, minTier + 1), maxTier, maxTier];
    return [];
}
function sanitizeOrderTiers(candidate, phase, maxDiscoveredTier, locationId) {
    ...
    if (!Array.isArray(candidate) || candidate.length !== expectedCount)
        return defaultOrderTiers(phase, maxTier, locationId);
...
function locationCurrentPhase(campaign, worldId, locationId) {
    if (!isCampaignWorldUnlocked(campaign, worldId))
        return null;
...
function createLocationRun(phase, maxDiscoveredTier, worldId = 1, locationId = SNEAKER_GARDEN_LOCATION_ID, landmarkLevel = 0) {
...
function phasePermanentProgress(campaign, worldId, locationId, phase) {
...
export function sanitizeCampaignRunState(candidate, campaign, maxDiscoveredTier) {
    if (candidate === null || candidate === undefined)
        return null;
    const raw = asRecord(candidate);
    if (!raw || typeof raw.worldId !== 'number' || typeof raw.locationId !== 'string')
        return null;
    const world = campaignWorldById(raw.worldId);
    if (!world || !campaignLocationById(world, raw.locationId))
        return null;
...
function locationLandmarkLevel(campaign, worldId, locationId) {
    const restore = campaignWorldProgress(campaign, worldId)?.locations[locationId]?.restore ?? 0;
    if (restore >= 1)
        return 3;
    if (restore >= 2 / 3)
...
function clearNearestOvergrowth(overgrowth, mergeIndex, count = 1) {
    ...
    for (const index of blocked.slice(0, Math.max(1, Math.floor(count))))
        next[index] = false;
    return { overgrowth: next, clearedIndex };
}
export function moveOrMergeCampaignRun(run, from, to, additionalOvergrowthClears = 0) {
...
export function abandonCampaignRun(state) {
...
export function restartCampaignRunPhase(state) {
    ...
    if (!progress)
        return state;
    let orderIndex = 0;
    if (current.phase === 'deliver')
        orderIndex = Math.floor(progress.deliver * SNEAKER_GARDEN_DELIVERY_ORDER_COUNT + 1e-9);
    if (current.phase === 'restore')
        orderIndex = Math.floor(progress.restore * SNEAKER_GARDEN_LANDMARK_LEVELS + 1e-9) * SNEAKER_GARDEN_RESTORE_BATCH_SIZE;
    if (current.phase === 'mastery')
        orderIndex = Math.floor(progress.mastery * SNEAKER_GARDEN_MASTERY_ORDER_COUNT + 1e-9);
