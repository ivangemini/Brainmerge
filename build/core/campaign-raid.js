...
export function sanitizeCampaignRaidRun(candidate, state) {
    if (!candidate || typeof candidate !== 'object')
        return null;
    const raw = candidate;
    if (typeof raw.worldId !== 'number' || (raw.phase !== 1 && raw.phase !== 2 && raw.phase !== 3))
        return null;
    ...
    const orderTiers = Array.isArray(raw.orderTiers) && raw.orderTiers.length === defaults.orderTiers.length
        ...
        : defaults.orderTiers;
    const merges = Math.max(0, Math.min(PHASE_MERGES[raw.phase], Number.isFinite(raw.merges) ? Math.floor(raw.merges) : 0));
    const orderIndex = Math.max(0, Math.min(orderTiers.length, Number.isFinite(raw.orderIndex) ? Math.floor(raw.orderIndex) : 0));
...
export function beginCampaignRaid(state, worldId) {
    if (state.campaignRun || state.raidRun)
        return state;
    const world = campaignWorldById(worldId);
    const progress = campaignWorldProgress(state.campaign, worldId);
    if (!world || !progress || !isWorldRaidUnlocked(world, progress) || progress.raidCleared)
        return state;
    const phase = Math.min(3, Math.floor(progress.raidProgress * 3) + 1);
    return { ...state, raidRun: createRun(worldId, phase, state.maxDiscoveredTier) };
}
export function spawnCampaignRaidSupply(state) {
...
export function selectCampaignRaidCell(state, index) {
    const run = state.raidRun;
    if (!run)
        return state;
    if (index === null || !Number.isInteger(index) || index < 0 || index >= BOARD_SIZE || run.overgrowth[index] || !run.cells[index])
        return { ...state, raidRun: { ...run, selectedIndex: null } };
    return { ...state, raidRun: { ...run, selectedIndex: run.selectedIndex === index ? null : index } };
}
export function moveOrMergeCampaignRaid(state, from, to) {
    const run = state.raidRun;
    if (!run || run.completed || run.overgrowth[from] || run.overgrowth[to])
        return state;
    ...
    if (!target) {
        cells[from] = null;
        cells[to] = source;
        return { ...state, raidRun: { ...run, cells, selectedIndex: null } };
    }
...
export function deliverCampaignRaidUnit(state, index) {
    const run = state.raidRun;
    if (!run || run.phase !== 3 || run.completed)
        return state;
    const target = run.orderTiers[run.orderIndex];
    if (!target || run.cells[index]?.tier !== target)
        return state;
    const cells = run.cells.slice();
    cells[index] = null;
...
export function acknowledgeCampaignRaidPhase(state) {
    const run = state.raidRun;
    if (!run?.completed)
        return state;
...
export function campaignRaidPresentation(run) {
    if (!run)
        return null;
