 ...
 */
export const INCOME_PER_MINUTE_BY_TIER = {
    1: 3,
    2: 7,
    3: 16,
    4: 36,
...
export const LUCKY_DROP_CHANCE_BY_LEVEL = [0, 0.03, 0.06, 0.09, 0.12, 0.15];
export const INCOME_MULTIPLIER_BY_LEVEL = [1, 1.05, 1.10, 1.15, 1.20, 1.25];
export const OFFLINE_HOURS_BY_LEVEL = [2, 4, 6, 8, 12];
...
export const DISCOVERY_BONUS_BY_TIER = {
    2: 0,
    3: 8,
    4: 12,
    5: 20,
...
const characterAsset = (_name) => BASE_CHARACTER_ATLAS;
/**
...
export function nextFamilyFor(familyId) {
    const family = familyById.get(familyId);
    if (!family)
        return null;
    return familyByTier.get(family.tier + 1) ?? null;
}
export function assetForUnit(familyId) {
    return familyById.get(familyId)?.asset ?? null;
}
export function mergeRewardForTier(tier) {
...
export function brainBoxCostForBaseTier(baseTier) {
...
export function luckyDropChanceForLevel(level) {
...
export function incomeMultiplierForLevel(level) {
